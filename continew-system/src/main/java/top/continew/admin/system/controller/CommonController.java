...
public class CommonController {
    ...
    public List<LabelValueResp> listUserDict(UserQuery query, SortQuery sortQuery) {
        ...
        List<LabelValueResp> resp = new ArrayList<>();
        for (UserDO item : list) {
            LabelValueResp labelValueResp = new LabelValueResp();
            labelValueResp.setLabel(item.getNickname());
            labelValueResp.setValue(item.getId().toString());
            resp.add(labelValueResp);
        }
        return resp;
    ...
    @SaIgnore
    @GetMapping("/dict/dept")
    public List<LabelValueResp> listDeptDict(DeptQuery query, SortQuery sortQuery) {
        return deptService.dict(query, sortQuery);
    }
}
